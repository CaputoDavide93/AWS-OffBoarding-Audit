...
def card(k, x, y, w, h, title, subs=(), *, mono=(), icon=None, tone="plain"):
    """k.box, with the sub-lines at the indices in `mono` set in monospace.

    File names are literally typed, so they take the monospace stack; the swap
    reuses the box's own position and colour.
    """
...
def architecture(scheme):
    k = Canvas(1180, 618, scheme,
               "Event History and CloudTrail Lake both write one event JSON file. The report "
               "reads it, with optional current state, a peer baseline, the pinned TrailDiscover "
               "catalogue and an external analysis, and writes the HTML dashboard. Every AWS call "
               "is a read.")
    W, H = 268, 108
    xs, top = [24, 456, 888], 56
    gap = 80
    r1, r2, r3 = top, top + H + gap, top + 2 * (H + gap)
    m1, m2 = r1 + H / 2, r2 + H / 2
    cx = [x + W / 2 for x in xs]
    lbl = dict(size=11.5, font=MONO, colour=k.c["chip"])
    k.add(
        # row 1: two collectors, one contract
        card(k, xs[0], r1, W, H, "Event History", ["aws_offboarding_audit.py",
             "every SSO account, last 90 days"], mono=[0], icon="cloud"),
        card(k, xs[1], r1, W, H, "Event JSON", ["aws_offboarding_audit.json",
             "one normalised event contract"], mono=[0], icon="file"),
        card(k, xs[2], r1, W, H, "CloudTrail Lake", ["aws_cloudtrail_lake.py",
             "a SQL query, or an export file"], mono=[0], icon="database"),
        # row 2: the report, its state input, its output
        card(k, xs[0], r2, W, H, "Current state", ["aws_current_state.py",
             "does the change still exist?"], mono=[0], icon="shield"),
        card(k, xs[1], r2, W, H, "Report", ["aws_audit_report.py",
             "detectors, review priority"], mono=[0], icon="chip", tone="accent"),
        card(k, xs[2], r2, W, H, "HTML dashboard", ["aws_offboarding_report.html",
             "plus Markdown and a summary"], mono=[0], icon="chart"),
        # row 3: optional inputs to the report
        k.box(xs[0], r3, W, H, "TrailDiscover", ["event catalogue, CC BY 4.0",
              "pinned commit, SHA-256 checked"], icon="globe"),
        card(k, xs[1], r3, W, H, "Peer baseline", ["audit_baseline.py",
             "three or more peer audits"], mono=[0], icon="users"),
        card(k, xs[2], r3, W, H, "Anthropic API", ["audit_analyst.py",
             "redacted digest, advisory"], mono=[0], icon="chat"),

        # collectors -> contract
        k.edge([(xs[0] + W + 8, m1), (xs[1] - 8, m1)], label="management events"),
        k.edge([(xs[2] - 8, m1), (xs[1] + W + 8, m1)], label="data events"),
        # contract -> report
        k.edge([(cx[1], r1 + H + 8), (cx[1], r2 - 8)]),
        k.text(cx[1] + 12, (r1 + H + r2) / 2 + 4, "events + manifest", **lbl),
        # contract -> current state -> report
        k.edge([(xs[1] + 44, r1 + H + 8), (xs[1] + 44, r1 + H + 40),
                (cx[0], r1 + H + 40), (cx[0], r2 - 8)]),
        k.text(cx[0] + 12, r1 + H + 64, "flagged targets", **lbl),
        k.edge([(xs[0] + W + 8, m2), (xs[1] - 8, m2)], label="--state"),
        # report -> dashboard
        k.edge([(xs[1] + W + 8, m2), (xs[2] - 8, m2)], label="one portable file"),
        # row 3 -> report
        k.edge([(xs[0] + W - 40, r3 - 8), (xs[0] + W - 40, r3 - 44),
                (xs[1] + 44, r3 - 44), (xs[1] + 44, r2 + H + 8)]),
        k.text(xs[0] + W - 28, r3 - 16, "catalogue", **lbl),
        k.edge([(cx[1], r3 - 8), (cx[1], r2 + H + 8)]),
        k.text(cx[1] + 12, r2 + H + 26, "--baseline", **lbl),
        k.edge([(xs[1] + W - 44, r2 + H + 8), (xs[1] + W - 44, r2 + H + 40),
                (cx[2], r2 + H + 40), (cx[2], r3 - 8)], dash=True, both=True),
        k.text(cx[2] + 12, r3 - 16, "--analyze", **lbl),

        k.footer("Every AWS call is a read (Get, List, Describe, LookupEvents, Lake queries); the dashboard "
                 "rebuilds from saved event JSON without querying AWS again."),
    )
    return k.render()
