...
REPORT_CONFIG_KEYS = {
    ...
    "state", "baseline", "sequence_hours", "no_enrich", "analyze", "search",
...
def load_report_config(path: str | None) -> dict:
    ...
    if "no_search" in defaults:  # pre-privacy-default config files
        defaults.setdefault("search", not defaults.pop("no_search"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    report_args.append("--search" if args.search else "--no-search")
    report_args.append("--redact" if args.redact else "--no-redact")
