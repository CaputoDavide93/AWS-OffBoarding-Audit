...
# ==========================================================================
# Pinned to a reviewed commit and verified by hash: the dataset shapes severity
# wording in the report, so a change upstream must be a deliberate bump here.
# To update: pick a new commit, download docs/events.json, review, update both.
TRAILDISCOVER_COMMIT = "f96cbdb0591f78d16b1948e84c211becce6258e6"
TRAILDISCOVER_SHA256 = "c16ffb8f99dfda1ea8ba2e9467e3dce9f1d783d03024f564ef1487e559016566"
TRAILDISCOVER_URL = ("https://raw.githubusercontent.com/adanalvarez/TrailDiscover/"
                     f"{TRAILDISCOVER_COMMIT}/docs/events.json")
CACHE_PATH = os.path.expanduser(
    f"~/.cache/aws-offboarding-audit/traildiscover-{TRAILDISCOVER_COMMIT[:12]}.json")
CACHE_TTL = 7 * 24 * 3600


def _verified(raw: bytes) -> bytes:
    digest = hashlib.sha256(raw).hexdigest()
    if digest != TRAILDISCOVER_SHA256:
        raise ValueError(f"TrailDiscover sha256 mismatch ({digest[:12]})")
    return raw


def load_traildiscover(refresh: bool = False, quiet: bool = False) -> dict[str, dict]:
    """Fetch the pinned TrailDiscover event corpus, cached locally for a week."""
    data = None
    if not refresh and os.path.exists(CACHE_PATH):
        if time.time() - os.path.getmtime(CACHE_PATH) < CACHE_TTL:
            try:
                with open(CACHE_PATH, "rb") as fh:
                    data = json.loads(_verified(fh.read()).decode("utf-8"))
            except (OSError, ValueError):
                data = None
    if data is None:
        try:
            with urllib.request.urlopen(TRAILDISCOVER_URL, timeout=25) as resp:
                raw = _verified(resp.read())
