...
def call_claude(digest: dict, api_key: str, model: str = DEFAULT_MODEL,
                use_search: bool = False, redact: bool = True, timeout: int = 300) -> dict:
...
def analyse(rows, sequences, ctx, api_key: str | None = None, model: str = DEFAULT_MODEL,
            use_search: bool = False, redact: bool = True) -> dict | None:
    key = api_key or os.environ.get("ANTHROPIC_API_KEY")
    if not key:
        print("  ! No API key. Set ANTHROPIC_API_KEY. Skipping analysis.",
              file=sys.stderr)
        return None
