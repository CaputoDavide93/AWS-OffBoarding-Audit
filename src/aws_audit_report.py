...
def main(argv: list[str] | None = None) -> int:
    ...
    # Privacy defaults: the digest carries a named employee's source IPs and
    # account data, so redaction is on and web search is off unless asked for.
    ap.add_argument("--search", action=argparse.BooleanOptionalAction, default=False,
                    help="Let the analyst use web search (off by default).")
    ap.add_argument("--redact", action=argparse.BooleanOptionalAction, default=True,
                    help="Hash account IDs and IPs before sending anything to the API "
                         "(on by default; --no-redact to send them in clear).")
    ...
    if args.analyze and rows:
        try:
            ...
            analysis = analyse(rows, sequences, ctx, model=args.model,
                               use_search=args.search, redact=args.redact)
